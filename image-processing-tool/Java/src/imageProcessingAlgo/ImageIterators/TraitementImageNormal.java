...
public class TraitementImageNormal extends TraitementImage {
	
	private double [] noyau = {0.0625,0.125,0.0625,0.125,0.25,0.125,0.0625,0.125,0.0625};
	int nbMedianRouge;
	int nbMedianVert;
	int nbMedianBleu;
		...
		for (int i = 1 ; i < L-1 ; i++) {
			for (int j = 1 ; j < l-1 ; j++) {
				if(TraitementImage.tab[j][i] == 0) {
					...
					img.setRGB(j, i, TraitementImage.copy.getRGB(j, i));
				}
			}	
		}
